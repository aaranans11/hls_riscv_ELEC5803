...
#define RESULT_ADDR ((volatile uint32_t*)0x0F00)
#define DONE_ADDR   ((volatile uint32_t*)0x0F04)
...
int main(void) {
  ...
  int32_t v[16];
  uint32_t spike[16*16];   // uint32_t so compiler uses SW, not SB
  for (int i = 0; i < 16; ++i) v[i] = 0;
  for (int i = 0; i < 16*16; ++i) spike[i] = 0;

  uint32_t seed = 1;

  for (int t = 0; t < T; ++t) {
    for (int n = 0; n < N; ++n) {
      int32_t in = (int32_t)(xorshift32(&seed) & 0x3F) - 31; // [-31..31]
